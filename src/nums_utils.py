...
def build_prompt_generator(cfg: dict, seed: int | None = None) -> PromptGenerator:
    ps = cfg["data_generation"]["prompt_set"]
    gen_seed = seed if seed is not None else cfg["data_generation"]["seed"]
    return PromptGenerator(
        rng=np.random.Generator(np.random.PCG64(gen_seed)),
        example_min_count=ps["example_min_count"],
        example_max_count=ps["example_max_count"],
        example_min_value=ps["example_min_value"],
        example_max_value=ps["example_max_value"],
        answer_count=ps["answer_count"],
        answer_max_digits=ps["answer_max_digits"],
    )
